...
from ..models import Tag, Photo_Tag
...
class TagDetailViewTest(APITestCase):
    def setUp(self):
        ...
        self.other_user_tag = Tag.objects.create(tag='다른유저태그', user=self.other_user)
...
class TagIntegrationTest(APITestCase):
    ...
    def test_multiple_users_tag_isolation(self):
        ...
        other_create_response = self.client.post(self.tag_list_url, {'tag': '유저1태그'})
        ...
        # 태그 ID는 달라야 함
        self.assertNotEqual(
            create_response.data['tag_id'],
            other_create_response.data['tag_id']
        )